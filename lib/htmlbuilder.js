...
module.exports.buildHtml = (auditData) => {
    ...
    var graphBuilder = fs.readFileSync(path.join(__dirname, './graph-data.js'), 'utf8');
      ...
      auditData.map((element) => {
        ...
        nodes.push({"id": (element.recommendation), "group": 6});
        }
      });
      auditData.map((element) => {
        ...
        nodes.push({"id": (element.vulnerable_versions), "group": 6});
        }
      });
      var links = auditData.map((element) => {
        var tempo = JSON.stringify(element.cves);
        if (tempo.length > 2) {
          return { "source": (tempo), "target": (element.cwe), "value": 9};
        } else {
          return { "source": ('NA'), "target": (element.cwe), "value": 9};
        }
      });
      auditData.map((element) => {
        links.push({ "source": (element.cwe), "target": (element.recommendation), "value": 9});
      });
      auditData.map((element) => {
        links.push({ "source": (element.title), "target": (element.recommendation), "value": 9});
      });
      auditData.map((element) => {
        links.push({ "source": (element.cwe), "target": (element.vulnerable_versions), "value": 9});
      });
      auditData.map((element) => {
        links.push({ "source": (element.title), "target": (element.vulnerable_versions), "value": 9});
      });
      auditData.map((element) => {
        links.push({ "source": (element.vulnerable_versions), "target": (element.recommendation), "value": 9});
      });
      auditData.map((element) => {
        var tempo = JSON.stringify(element.cves);
        if (tempo.length > 2) {
          return { "source": (tempo), "target": (element.title), "value": 9};
        } else {
          return { "source": ('NA'), "target": (element.title), "value": 9};
        }
      });
      auditData.map((element) => {
        var tempo = JSON.stringify(element.cves);
        if (tempo.length > 2) {
          return { "source": (tempo), "target": (element.recommendation), "value": 9};
        } else {
          return { "source": ('NA'), "target": (element.recommendation), "value": 9};
        }
      });
      auditData.map((element) => {
        var tempo = JSON.stringify(element.cves);
        if (tempo.length > 2) {
          return { "source": (tempo), "target": (element.vulnerable_versions), "value": 9};
        } else {
          return { "source": ('NA'), "target": (element.vulnerable_versions), "value": 9};
        }
      });
