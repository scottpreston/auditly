...
module.exports.buildHtml = (auditData) => {
    ...
    var graphBuilder = fs.readFileSync(path.join(__dirname, './graph-test.js'), 'utf8');
      ...
      var nodes = auditData.map((element) => {
        return { "id": (element.cves), "group": 1};
      });
      auditData.map((element) => {
        nodes.push({"id": (element.cwe), "group": 2});
      });
      auditData.map((element) => {
        nodes.push({"id": (element.title), "group": 3});
      });
      var links = auditData.map((element) => {
        return { "source": (element.cves), "target": (element.cwe), "value": 5};
      });
      auditData.map((element) => {
        links.push({ "source": (element.cwe), "target": (element.title), "value": 5});
      });
